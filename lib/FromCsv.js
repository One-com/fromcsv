...
module.exports = function FromCsv(options) {
  ...
  function getDataPresenceFromRows(csvRows) {
    ...
    csvRows.forEach(row => {
      var foundIndexes = findAllIndexes(row, elem => elem && elem.length > 0);
  ...
  function matchHeaderByHash(matchHash, headerRow) {
    ...
    headerRow.forEach(function(columnName, columnIndex) {
      ...
      if (matches) {
        seen[columnName] = seen[columnName] || 0;

        let matchedName;

        const isColumnAmbiguous = Array.isArray(matches);
        if (isColumnAmbiguous) {
          // use the seen information to index the matches
          const seenIndex = seen[columnName];
          matchedName = matches[seenIndex];
        } else {
          matchedName = matches;
        }

        // record that the column was seen
        seen[columnName] += 1;
  ...
  function processRow(csvConfig, headerInfo, dataRow) {
    ...
    var unmatchedNamesByIndex = headerInfo.unmatchedColumnsByIndex;
    ...
    dataRow.forEach(function(value, columnIndex) {
      ...
      if (isUnmapped) {
        columnName = unmatchedNamesByIndex[columnIndex] || columnIndex;
      }
    ...
    return processRowCoalesce(contact, aliases, unknowns, headerInfo);
  }
